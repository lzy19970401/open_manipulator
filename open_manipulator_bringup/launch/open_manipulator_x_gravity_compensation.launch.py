...
#
# OpenMANIPULATOR-X Gravity compensation control mode launch.
#
# Mutually exclusive with Standard control launch (open_manipulator_x.launch.py):
# do not run both at the same time. They claim different command interfaces on
# the same Arm joints (effort vs position) and must not be loaded together.
#
# Hardware (or mock hardware CI smoke):
...
#
# Gazebo simulation (separate launch — spawns gz_ros2_control + effort transmission):
#   ros2 launch open_manipulator_bringup open_manipulator_x_gravity_compensation_gazebo.launch.py
#
# Shutdown safety (hardware only): on exit the gravity compensation controller zeros Arm
# effort commands, then gc_shutdown_torque_disable disables torque on Arm actuators
# (Dynamixel IDs 11-14). Wait for launch shutdown to finish before manually moving the Arm.
