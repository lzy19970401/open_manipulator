...
#
# OpenMANIPULATOR-X Pinocchio task-space impedance control mode — Gazebo simulation.
#
# Mutually exclusive with Standard control Gazebo launch
# (open_manipulator_x_gazebo.launch.py), joint-space impedance Gazebo launch,
# and GC Gazebo launches: do not run both at once.
#
...
#   1. Wait until pinocchio_task_impedance_controller is active (spawner exits cleanly).
#   2. With capture_on_activate (default), the current end-effector position is x_d.
#   3. Drag the arm in Gazebo (GUI joint handles or apply external force on the EE link).
#   4. Release: the arm should spring back toward x_d with damping (no excessive ringing).
#   5. Ctrl+C to stop; no hardware torque-disable step is needed in simulation.
#
...
def generate_launch_description():
    ...
    doc = xacro.process_file(
        ...
        mappings={
            ...
            'config_type': 'open_manipulator_x_task_impedance_pinocchio',
            'use_effort_transmission': 'true',
        },
    )
    ...
    task_impedance_controller_spawner = Node(
        package='controller_manager',
        executable='spawner',
        arguments=[
    ...
    return LaunchDescription([
        ...
        RegisterEventHandler(
            event_handler=OnProcessExit(
                ...
                on_exit=[task_impedance_controller_spawner],
            )
        ),
        bridge,
